...
enum class HttpHeaders {
     ...
     * A "host" without any trailing port information implies the default port for the service requested (e.g., "80" for an HTTP URL). For example, a request on the origin server for <http:www.w3.org></http:www.w3.org>/pub/WWW/> would properly include:
     *
     *
     ...
     */
    fun parse(slice: ByteBuffer): ByteBuffer {
        slice.position(tokenLen + 2 + slice.position())
        while (Character.isWhitespace(slice[slice.limit() - 1].toInt())) slice.limit(slice.limit() - 1)
        return slice
    }
